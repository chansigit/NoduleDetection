...
output="/data640/TCIA-LIDC-IDRI_lungExtracted"
    ...
    if not dirs:
        if len([f for f in os.listdir(root) if os.path.isfile( os.path.join(root,f) )]) >20:
            ...
            for imgID in xrange(len(patientImg)):
                lungImg = get_segmented_lungs(im=patientImg[imgID], plot=False, THRESHOLD=-320)
                ...
                sliceName = os.path.join(sliceName, "%s.pickle" % slicePos)
                pickle.dump(lungImg, open(sliceName,"wb"))
                print(sliceName)
